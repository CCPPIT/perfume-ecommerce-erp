// صفحة إدارة عطور النيش الفاخرة - قسم متخصص في فئة المنتجات والعطور
...
export default function NicheLuxuryPerfumesPage() {
  const [currentCategory, setCurrentCategory] = useState('products-perfumes');
  const [currentDepartment, setCurrentDepartment] = useState('إدارة عطور النيش الفاخرة');
    ...
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  إدارة شاملة لعطور النيش الفاخرة • قسم متخصص في الفخامة والحصرية
                </p>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {activeTab === 'overview' && <NicheLuxuryPerfumesOverviewTab />}
...
// نظرة عامة على عطور النيش الفاخرة
function NicheLuxuryPerfumesOverviewTab() {
  const perfumeStats = [
    {
      title: 'إجمالي عطور النيش الفاخرة',
      value: '456 عطر',
      change: '+34 هذا الشهر',
      trend: 'up',
      icon: Package,
  ...
  const perfumeCategories = [
    { category: 'العطور الشرقية الفاخرة', count: 123, percentage: 27, color: 'amber' },
    { category: 'العطور الفرنسية النادرة', count: 98, percentage: 22, color: 'pink' },
    { category: 'العطور الإيطالية الفاخرة', count: 87, percentage: 19, color: 'green' },
    { category: 'العطور العربية النادرة', count: 76, percentage: 17, color: 'orange' },
    { category: 'العطور اليابانية الفاخرة', count: 45, percentage: 10, color: 'blue' },
    { category: 'العطور الأمريكية الحصرية', count: 27, percentage: 5, color: 'red' }
  ];
  ...
  return (
    <div className="space-y-8">
            ...
            {perfumeCategories.map((cat, index) => (
              ...
              >
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center bg-${cat.color}-100 dark:bg-${cat.color}-900/30`}>
                  <Diamond className={`w-6 h-6 text-${cat.color}-600`} />
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <TikTokCard className="p-6">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">
            خط تطوير عطور النيش الفاخرة
          </h3>

          <div className="space-y-4">
            {[
              { stage: 'البحث عن المكونات النادرة', count: 12, status: 'نشط', color: 'purple' },
              { stage: 'اختبار التركيبات الحصرية', count: 8, status: 'قيد المراجعة', color: 'yellow' },
              { stage: 'مراقبة الجودة الفاخرة', count: 5, status: 'معتمد', color: 'green' },
              { stage: 'التعبئة بالمواد الفاخرة', count: 7, status: 'قيد التنفيذ', color: 'pink' }
            ].map((stage, index) => (
              <motion.div
                key={index}
                className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-xl"
              ...
              >
                <div className={`w-10 h-10 bg-${stage.color}-100 dark:bg-${stage.color}-900/30 rounded-lg flex items-center justify-center`}>
                  <Crown className={`w-5 h-5 text-${stage.color}-600`} />
                </div>
        ...
        <TikTokCard className="p-6">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-6">
            مؤشرات الجودة لعطور النيش الفاخرة
          </h3>

          <div className="space-y-6">
            {[
              { metric: 'معدل الرضا عن الفخامة', value: 98.2, target: 95, status: 'ممتاز' },
              { metric: 'نسبة المنتجات الحصرية', value: 96.5, target: 90, status: 'ممتاز' },
              { metric: 'زمن اختبار الجودة الفاخرة', value: 4.5, target: 5.0, status: 'ممتاز' },
              { metric: 'تكلفة مراقبة الجودة', value: 3.2, target: 3.0, status: 'جيد جداً' }
            ].map((metric, index) => (
              <motion.div
                key={index}
                className="space-y-2"
    ...
    <TikTokCard className="p-8">
      <div className="text-center py-16">
        ...
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
          إدارة منتجات عطور النيش الفاخرة
        </h3>
        <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
          نظام شامل لإدارة جميع منتجات عطور النيش الفاخرة والحصرية
        </p>
  ...
  return (
    <TikTokCard className="p-8">
      <div className="text-center py-16">
        ...
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
          تطوير عطور النيش الفاخرة
        </h3>
        <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
          تطوير عطور نيش فاخرة جديدة مع الحفاظ على الفخامة والحصرية
        </p>
        <TikTokButton size="lg" variant="outline">
          <Crown className="w-5 h-5 mr-2" />
          مشروع تطوير فاخر جديد
        </TikTokButton>
      </div>
    </TikTokCard>
  );
  ...
  return (
    <TikTokCard className="p-8">
      <div className="text-center py-16">
        ...
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
          مراقبة جودة عطور النيش الفاخرة
        </h3>
        <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
          ضمان جودة عطور النيش الفاخرة وتطبيق معايير الفخامة العالية
        </p>
        <TikTokButton size="lg">
          <Award className="w-5 h-5 mr-2" />
          مراجعة جودة فاخرة جديدة
        </TikTokButton>
      </div>
    </TikTokCard>
  );
  ...
  return (
    <TikTokCard className="p-8">
      <div className="text-center py-16">
        ...
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
          تقارير عطور النيش الفاخرة والأداء
        </h3>
        <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
          تقارير مفصلة عن أداء عطور النيش الفاخرة والمبيعات والجودة والتطوير
        </p>
        <TikTokButton size="lg" variant="outline">
  ...
  return (
    <TikTokCard className="p-8">
      <div className="text-center py-16">
        ...
        <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
          إعدادات إدارة عطور النيش الفاخرة
        </h3>
        <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-2xl mx-auto">
          تخصيص إعدادات نظام إدارة عطور النيش الفاخرة حسب احتياجات المؤسسة
        </p>
        <TikTokButton size="lg">
          <Settings className="w-5 h-5 mr-2" />
          تعديل الإعدادات الفاخرة
        </TikTokButton>
      </div>
    </TikTokCard>
  );
